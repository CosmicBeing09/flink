...
class SinkV2SinkWriterOperatorTest {
    ...
    InspectableSink sinkWithoutCommitter() {
        TestSinkV2.DefaultSinkWriter<Integer> defaultSinkWriter = new TestSinkV2.DefaultSinkWriter<>();
        return new InspectableSink(TestSinkV2.<Integer>newBuilder().setWriter(defaultSinkWriter).build());
    }

    InspectableSink sinkWithCommitter() {
        TestSinkV2.DefaultSinkWriter<Integer> committingSinkWriter =
                new TestSinkV2.DefaultCommittingSinkWriter<>();
        return new InspectableSink(
                TestSinkV2.<Integer>newBuilder()
                        .setWriter(committingSinkWriter)
                        .setDefaultCommitter()
                        .build());
    }

    InspectableSink sinkWithTimeBasedWriter() {
        TestSinkV2.DefaultSinkWriter<Integer> timeBasedBufferingSinkWriter = new TimeBasedBufferingSinkWriter();
        return new InspectableSink(
                TestSinkV2.<Integer>newBuilder()
                        .setWriter(timeBasedBufferingSinkWriter)
                        .setDefaultCommitter()
                        .build());
    }

    InspectableSink sinkWithState(boolean withState, String stateName) {
        TestSinkV2.DefaultSinkWriter<Integer> statefulSinkWriter =
                new TestSinkV2.DefaultStatefulSinkWriter<>();
        TestSinkV2.Builder<Integer> builder =
                TestSinkV2.<Integer>newBuilder()
                        .setDefaultCommitter()
                        .setWithPostCommitTopology(true)
                        .setWriter(statefulSinkWriter);
        if (withState) {
