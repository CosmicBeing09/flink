...
/** {@link FlinkPipelineTranslator} for DataSet API {@link StreamGraphPlan Plans}. */
public class PlanTranslator implements FlinkPipelineTranslator {
    ...
    public JobGraph translateToJobGraph(
        ...
        checkArgument(pipeline instanceof StreamGraphPlan, "Given pipeline is not a DataSet Plan.");
    ...
    private void setDefaultParallelism(StreamGraphPlan plan, int defaultParallelism) {
    ...
    public String translateToJSONExecutionPlan(StreamGraph pipeline) {
        ...
        OptimizedStreamGraph optPlan = opt.compile(plan);

        return new StreamGraphJSONDumpGenerator().getOptimizerPlanAsJSON(optPlan);
    }

    private JobGraph compilePlan(StreamGraphPlan plan, Configuration optimizerConfiguration) {
        Optimizer optimizer = new Optimizer(new DataStatistics(), optimizerConfiguration);
        OptimizedStreamGraph optimizedPlan = optimizer.compile(plan);
    ...
    public boolean canTranslate(StreamGraph pipeline) {
        return pipeline instanceof StreamGraphPlan;
    }
}
