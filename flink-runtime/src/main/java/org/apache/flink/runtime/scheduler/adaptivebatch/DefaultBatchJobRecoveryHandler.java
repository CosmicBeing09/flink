...
public class DefaultBatchJobRecoveryHandler
    ...
    private void restoreOperatorCoordinators(
            Map<OPERATOR_ID_PAIR, byte[]> snapshots, Map<OPERATOR_ID_PAIR, JobVertexID> operatorToJobVertex)
            throws Exception {
        for (Map.Entry<OPERATOR_ID_PAIR, byte[]> entry : snapshots.entrySet()) {
            OPERATOR_ID_PAIR operatorId = entry.getKey();
    ...
    public void finalizeReplay() throws Exception {
        ...
        final Map<OPERATOR_ID_PAIR, JobVertexID> operatorToJobVertex = new HashMap<>();
    ...
    public void onExecutionFinished(ExecutionVertexID executionVertexId) {
        ...
        final Map<OPERATOR_ID_PAIR, CompletableFuture<byte[]>> operatorCoordinatorSnapshotFutures =
                new HashMap<>();
    ...
    private Map<OPERATOR_ID_PAIR, CompletableFuture<byte[]>> snapshotOperatorCoordinators() {

        final Map<OPERATOR_ID_PAIR, CompletableFuture<byte[]>> snapshotFutures = new HashMap<>();
