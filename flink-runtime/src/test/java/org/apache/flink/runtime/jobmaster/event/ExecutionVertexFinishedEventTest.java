...
class ExecutionVertexFinishedEventTest {
    ...
    void testExecutionVertexFinishedEventSerializer() throws Exception {
        ...
        final Map<OPERATOR_ID_PAIR, byte[]> operatorCoordinatorSnapshots = new HashMap<>();
        byte[] bytes = new byte[2000];
        for (int i = 0; i < 3; ++i) {
            new Random(i).nextBytes(bytes);
            operatorCoordinatorSnapshots.put(new OPERATOR_ID_PAIR(), bytes);
        }
    ...
    private static boolean snapshotsEquals(
            Map<OPERATOR_ID_PAIR, CompletableFuture<byte[]>> snapshotsA,
            Map<OPERATOR_ID_PAIR, CompletableFuture<byte[]>> snapshotsB)
            throws Exception {
        return mapEquals(
                snapshotsA,
                snapshotsB,
