...
class StateAssignmentOperationTest {
    ...
    private void verifyOneKindPartitionableStateRescale(
            OperatorState operatorState, OperatorIDPair operatorID) {
        // rescale up case, parallelism 2 --> 3
    ...
    void testChannelStateAssignmentStability() throws JobException, JobExecutionException {
        ...
        List<OperatorIDPair> operatorIds = buildOperatorIds(numOperators);
        Map<OperatorIDPair, ExecutionJobVertex> vertices =
                buildVertices(operatorIds, numSubTasks, RANGE, ROUND_ROBIN);
        Map<OperatorIDPair, OperatorState> states = buildOperatorStates(operatorIds, numSubTasks);
    ...
    void testChannelStateAssignmentDownscalingTwoDifferentGates()
        ...
        JobVertex upstream1 = createJobVertex(new OperatorIDPair(), 2);
        JobVertex upstream2 = createJobVertex(new OperatorIDPair(), 2);
        JobVertex downstream = createJobVertex(new OperatorIDPair(), 2);
        List<OperatorIDPair> operatorIds =
                Stream.of(upstream1, upstream2, downstream)
                        .map(v -> v.getOperatorIDPairs().get(0).getGeneratedOperatorID())
                        .collect(Collectors.toList());
        Map<OperatorIDPair, OperatorState> states = buildOperatorStates(operatorIds, 3);
    ...
    public void testChannelStateAssignmentTwoGatesPartiallyDownscaling()
        ...
        JobVertex upstream1 = createJobVertex(new OperatorIDPair(), 2);
        JobVertex upstream2 = createJobVertex(new OperatorIDPair(), 2);
        JobVertex downstream = createJobVertex(new OperatorIDPair(), 3);
        List<OperatorIDPair> operatorIds =
                Stream.of(upstream1, upstream2, downstream)
                        .map(v -> v.getOperatorIDPairs().get(0).getGeneratedOperatorID())
                        .collect(Collectors.toList());
    ...
    void testChannelStateAssignmentDownscaling() throws JobException, JobExecutionException {
        List<OperatorIDPair> operatorIds = buildOperatorIds(2);
        Map<OperatorIDPair, OperatorState> states = buildOperatorStates(operatorIds, 3);
    ...
    void testChannelStateAssignmentNoRescale() throws JobException, JobExecutionException {
        List<OperatorIDPair> operatorIds = buildOperatorIds(2);
        Map<OperatorIDPair, OperatorState> states = buildOperatorStates(operatorIds, 2);
    ...
    void testOnlyUpstreamChannelStateAssignment() throws JobException, JobExecutionException {
        ...
        List<OperatorIDPair> operatorIds = buildOperatorIds(2);
        Map<OperatorIDPair, OperatorState> states = new HashMap<>();
    ...
    void testStateWithFullyFinishedOperators() throws JobException, JobExecutionException {
        List<OperatorIDPair> operatorIds = buildOperatorIds(2);
        Map<OperatorIDPair, OperatorState> states =
    ...
    void assigningStatesShouldWorkWithUserDefinedOperatorIdsAsWell() {
        int numSubTasks = 1;
        OperatorIDPair operatorId = new OperatorIDPair();
        OperatorIDPair userDefinedOperatorId = new OperatorIDPair();
        List<OperatorIDPair> operatorIds = singletonList(userDefinedOperatorId);
    ...
    void assigningStateHandlesCanNotBeNull() {
        OperatorState state = new OperatorState(new OperatorIDPair(), 1, MAX_P);
    ...
    private List<OperatorIDPair> buildOperatorIds(int numOperators) {
        return IntStream.range(0, numOperators)
                .mapToObj(j -> new OperatorIDPair())
                .collect(Collectors.toList());
    }

    private Map<OperatorIDPair, OperatorState> buildOperatorStates(
            List<OperatorIDPair> operatorIDs, int numSubTasks) {
        Random random = new Random();
        final OperatorIDPair lastId = operatorIDs.get(operatorIDs.size() - 1);
    ...
    private static class OperatorIdWithParallelism {
        ...
        public OperatorIdWithParallelism(OperatorIDPair operatorID, int parallelism) {
            this.operatorID = operatorID;
            this.parallelism = parallelism;
        }
    }

    private Map<OperatorIDPair, ExecutionJobVertex> buildVertices(
            List<OperatorIDPair> operatorIds,
            int parallelisms,
    ...
    private Map<OperatorIDPair, ExecutionJobVertex> toExecutionVertices(JobVertex... jobVertices)
            throws JobException, JobExecutionException {
        ...
        return Arrays.stream(jobVertices)
                .collect(
                        Collectors.toMap(
                                jobVertex ->
                                        jobVertex.getOperatorIDPairs().get(0).getGeneratedOperatorID(),
                                jobVertex -> {
                                    try {
    ...
    private ExecutionJobVertex buildExecutionJobVertex(
            OperatorIDPair operatorID, OperatorIDPair userDefinedOperatorId, int parallelism) {
        try {
    ...
    private JobVertex createJobVertex(OperatorIDPair operatorID, int parallelism) {
    ...
    private JobVertex createJobVertex(
        ...
        JobVertex jobVertex =
                new JobVertex(
                        ...
                        new JobVertexID(),
                        singletonList(org.apache.flink.runtime.OperatorIDPair.of(operatorID, userDefinedOperatorId)));
    ...
    private OperatorSubtaskState getAssignedState(
            ExecutionJobVertex executionJobVertex, OperatorIDPair operatorId, int subtaskIdx) {
