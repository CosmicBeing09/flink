...
import static org.apache.flink.state.forst.ForStStateBackend.PRIMARY_SHORTCUT_CHECKPOINT;
...
class ForStStateBackendV2Test extends StateBackendTestV2Base<ForStStateBackend> {
    ...
    void testRemoteDirShareCheckpointDir(boolean createJob) throws Exception {
        ...
        Configuration config = new Configuration();
        config.set(LOCAL_DIRECTORIES, tempFolderForForStLocal.toString());
        config.set(PRIMARY_DIRECTORY, PRIMARY_SHORTCUT_CHECKPOINT);
        ...
        assertThat(keyedBackend.getPrimaryBasePath().getParent())
                .isEqualTo(
                        new Path(
